...
LTO 管的是**一件大活从想清楚到上线的全过程纪律**——README 讲怎么用，这份讲怎么装、装到什么程度。

它靠三道闸门踩刹车：**这事是不是做早了**（说不出现在缺什么信号，就是早了）、**数据到没到线**（先定阈值再看数，不事后改标准）、**人拍没拍板**（该停不停由你决定，不由 AI 自己收敛）。推进过程中，它派**别家模型**来挑刺（写代码的不审自己的代码），报告一律回源码核对而不是照单全收；上线严格按 schema → dry-run → 只读 → 实测的顺序走；每个拍板点当场记下反例和天花板。

**最小可用是零依赖的**：三道闸门、核验否决这些纯方法论部分，拷过去就能用。另有三个可选插槽——异构审计、记忆落盘、派活给别的 AI——按需装配，装多少用多少，不装也不影响主流程。
