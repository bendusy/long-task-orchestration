...
`--backend` 未显式指定时自动检测宿主所在的 multiplexer：`$TMUX` 存在选 tmux，`$HERDR_ENV`/`$HERDR_SOCKET_PATH` 存在选 herdr，都没有回落 tmux；显式传参覆盖检测。herdr backend 下 LTO 先探测 `herdr status`；server 未运行即 fail-closed，并提示启动 Herdr 或改用 tmux backend。Herdr 的 `agent wait`/`agent prompt` idle、done 只作启动就绪与传输状态，C5 `goal-self-report` 才是唯一主完成信号；blocked 仍由 LTO 自己对 `herdr pane read` 套 `blocked_patterns` 判定。
