...
**派工窗口默认保留**：runner 的正文只活在终端 scrollback 里，dispatch 记录只存启动片段（`turns_jsonl` 为空、`capture_excerpt` 仅数百字节），成功即关窗等于把产出丢掉。要关窗须显式 `--close-window`；`--keep-window` 保留为兼容用的 no-op。窗口留着时，产出用 `orca terminal read --limit <n>` / `tmux capture-pane` 自行抢救。
