...
**Dispatch & execution**: `scheduler.rs` (concurrent job scheduler: submit/retry/healthcheck/deps), `dispatch.rs` (capability-scored runner selection), `dispatch_goal.rs` (`dispatch-goal` → codex/pi/agy via tmux, primary completion=`goal-self-report`; Stop hook/process-exit optional side-channels), `agent_job.rs` (`AgentJob`/`AgentResult`/`Sandbox`/`PermissionPolicy`), `agent_turn.rs` (turn vs dispatch completion routing + owned-window cleanup), `process.rs` (git/shell command factory), `worktree.rs` (worktree lifecycle), `tmux_runner.rs` (immutable `@window_id` targeting; Signal/Sentinel/Fire modes).
