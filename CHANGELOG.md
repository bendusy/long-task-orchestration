...
- **新增 orca 与 paseo dispatch backend**：`--backend` 现有 tmux/herdr/orca/paseo 四档。自动检测顺序为 `$ORCA_TERMINAL_HANDLE`/`$ORCA_WORKSPACE_ID` → `$HERDR_ENV`/`$HERDR_SOCKET_PATH` → tmux；Paseo 不往它起的终端注入自己的标记，需显式 `--backend paseo`。检测逻辑集中到新的 `src/host_env.rs`，派工选的 backend 与 `lto start` 记录的 host 出自同一次探测。
- **GUI multiplexer 优先于内层 tmux**：在 Orca 标签页里再开 tmux 时，派工落到 Orca 标签页而非 tmux 窗口——用户看的是标签页，派进内层 tmux 要 attach 才看得见。裸 tmux（外面没有 GUI）不受影响。两个 id 都会记进 `WorkspaceSnapshot.extra`。
- **agy 的首次信任提示现在按「被阻塞」识别**：Antigravity CLI 首次在某目录运行会弹 `Do you trust the contents of this project?` 并等回车，而 `blocked_patterns` 此前只给 codex 配了闸门正则，agy 一条都没有——派工因此看不出它被卡住，只能干等满 60s ready timeout，再报一个「没就绪」而非「被交互提示卡住」的错误原因。现在 agy 的信任提示会被当场识别并给出可操作提示（在窗口里回答，或改用走 `--print` 的 runner 脚本）。各家 CLI 的闸门措辞互不通约，按 runner 分别登记。
- **已知边界：orca 终端的产出不耐 runner 退出**。runner 进程一退出，orca 即丢弃该终端 scrollback：`terminal show` 仍列出 handle 并标 `orphaned: true`，但 `terminal read` 返回 0 行，只剩一行 `preview`。跑完即退的 runner（codex）事后无从捞回，停在 TUI 的（agy）才读得到。派给 orca 的 goal 应要求把结果写入文件；tmux backend 无此问题。
...
- **写事件前必须存在已落盘的 run**。`events::emit` 现在要求 `state.json` 存在，否则拒绝写入并报错。`start` 不受影响，它本来就先存 state 再发 `run.started`。实测：拿一个不存在的 run-id 跑 `plugin eval-run`，以前会建出完整目录树（events、telemetry、十来个 plugin-eval 产物）且返回码为 0，现在直接拒绝，目录一个都不建。
- **四处「写失败就当没发生」的调用改成会报错**。它们当初吞掉错误，是因为没有 run 时压根无处可写；闸门加上之后这个理由不成立了，那个不检查返回值的 emitter 已删除。
- **`dispatch-goal --cwd` 会先检查目录是否存在**。以前这个参数只是被拼进 `cd` 命令发给 tmux，而发送方不看 shell 执行的结果——路径打错时 shell 停在上一行的仓库根，后面的启动命令照发，agent 就在错误的目录里干活并留下真实改动。现在派工之前就拒绝，连窗口都不会开。
- **一条投票格式不对，不再让整个 runner 的票作废**。`decision` 解析多 runner 意见时，循环里的 `?` 会因单条不合 schema 而丢掉该 runner 的全部投票，于是「投了但有一条格式错」和「根本没投」变得无法区分——这直接改变表决分母，可能翻转结论。现在坏条目单独跳过并计数，好的保留；全坏才回落到文本兜底。刻意没做的：不给结构体加 `serde(default)` 硬吞，那会把错误数据变成有效票，比丢票更糟。
- **并发保存不再互相覆盖**。`save_state_preserving_c2` 在锁里重读了磁盘状态，却只把五个 contract 字段搬过来，其余一律用调用方手里的旧快照整体写回。两个进程各加一个 dispatch window，后写的那个会把先写的抹掉；agent_runs、risk_points、phase transitions、decision 记录都在暴露范围内。现在按字段语义分类合并：追加型集合按 key 取并集且顺序稳定，`tasks`/`gates`/`budget` 这类调用方刚算过的仍是后写者优先（否则删除会失效）。
- **窗口被保留时会说清楚原因**。保留有五条路径，措辞各不相同；最糟的是重复自报那条——窗口一旦标记为 `retained`，第二次上报就匹配不上「活跃」记录，落到「找不到该 runner 的活跃派工记录」，读起来像记录丢了。其实 `status` 和 `retention_reason` 一直好好写在状态文件里。现在这条分支会把查到的真实状态和原因报出来，「已经处理过」和「确实没有记录」终于能分辨。另外清理出错时也会落盘，不再留下一个状态还是「活跃」、实际却没人管的窗口。
- **隐藏命令纳入文档一致性检查**。`parallel` 和 `pipeline` 可以调用、接受 `--run-id`，却既不在命令清单、也不在文档、也不在 `--help` 里，检查脚本完全看不见它们。现在补齐并标注为隐藏；同时新增一道闸门：解析 `Commands` 枚举的全部变体（含隐藏的）与清单比对，漏了就红。这道闸门当场揪出另外三个隐藏变体（`task-add`、`task-update`、`phase`，都是 `task` 子命令的历史别名），已显式登记而非默默容忍。

另有两项此前累积未发布的派工能力：**codex 的 goal 改经 `/goal` 下发**以启用其 goal-runtime；**新增 aix 作为第四个 dispatch-goal runner**。

**升级注意**：如果你有脚本依赖「给任意 run-id 就能写事件」的旧行为（例如先写事件再补 state），需要调整顺序——先 `start`，再写事件。`notify_cmd` 有一处行为收窄：当前 schema 无法区分「没改」和「清空」，保存路径统一按「没改」处理，因此不能再通过它显式清空 notify 命令。

## v0.11.2 — 人工批准有了「保质期」，空转不再吃预算（2026-08-03）

长任务跑几天后有个隐患：三天前你批准过的事，代码和阶段早就变了，但批准还被当作有效。这版给人工裁决加上锚点和漂移检查，同时修掉一批派工实战里踩出来的坑。机制设计源自与同类开源项目 LoopX 的对比评审——四个候选机制经异构合议只有这一个活了下来，其余的要么重复已有能力、要么违背薄 harness 原则，被否了。

- **新增 `lto decision` 命令族**：`record` 把一条人工裁决连同当时的 HEAD 和阶段一起记下来；之后 `resume`/`check` 发现代码或阶段漂移了，会列出哪些旧裁决需要重新确认，并给出现成的 `reaffirm` 命令。刻意不做的：不按天数过期（那只会造成确认疲劳）、不自动作废、更不自动替你重新批准——列出来，决定权还是你的。旧格式的裁决记录原样保留，单独分组提示。
- **autopilot 不再为空转记账**：以前每次调用一进门就 `turns_used +1`，哪怕随后被预算拦下、被自主门禁退回、或根本没有可执行的任务。现在只有真正开始执行动作才计数，预算判定也改用未加一的值——预算余量不会再被自己的检查动作吃掉。
- **audit 派工会跳过连续掉线的 runner**：codex 满载、pi 429 是常态。同一个 run 里某家连挂两次，下次派工自动跳过并说明原因；你用 `--prefer-runner` 点名时照旧派（点名优先于回避）。只看本 run 的记录，不搞跨 run 黑名单。
- **派工不再误伤忙窗口**：不指定目标时，候选窗格若有前台进程在跑，自动改开新窗口；显式指定的目标忙着，则直接报错并告诉你换 `--new-window`——不会再把 goal 注进别人正在用的终端。
- **closeout 两处更早说话**：工作树有未提交改动时，进门就警告并提示 `--no-changelog` 的正确用法，不等到被拦下才知道；人工验收提示里加了三句自查——你的验收脚本本身验过吗（测的是新增还是修改？目标真的 active 吗？写入确认成功了吗）——上次的误报全是验收脚本自己的锅。

顺带修复：`decision record` 在非 Git 目录下解析不到 HEAD 时会当场警告（仍允许写入，锚点标 unknown，宁可多提醒不静默）。

四轮精简的收尾。**没有新功能、没有命令或参数变化、`.lto/` 文件协议和 gate 语义全部不变**——升级只是让代码少一些重复。

- **shell 单引号转义现在只有一份定义**（`process.rs`）。此前散在四个文件里，实现逐字节相同。这是安全相关的转义，四份意味着修一处漏三处；其中第四份因为函数名不同（`shell_quote` vs `shell_single_quote`），上一轮按名字搜索时漏掉了。
- **job-file 的执行流程合并成一处**。`runner --job-file`、`run parallel`、`run pipeline` 三条路径此前各抄了一遍"发 started 事件 → 提交 → 失败发事件 → 结果落 state"，只有事件标签不同。任一处漏改都会与另两处静默分叉，现在不会了。
- **补上了这三条路径的事件测试**。此前删掉整个 started 事件，测试依然全绿——它们只核了结果落 state，不看生命周期事件。现在把事件标签写错会当场变红。
- **`dispatch-and-wait` 的等待逻辑挪进了 `dispatch_goal`**。它原本住在命令行分发表里，而它要用的窗口保留函数在另一个模块。
- 顺带：删掉七处多余的值拷贝、一个只为判空就克隆整批数据的检查、一个永不失败却要求调用方处理错误的构造函数。
