...
| `dispatch-goal` | Dispatch a goal file to codex, pi, or agy through tmux by default; pass `--backend herdr` for an explicit Herdr tab/pane and print a ready-to-copy completion wait command. | `--runner <runner> --goal <path>` | `--backend tmux\|herdr`, `--run-id`, `--target`, `--new-window`, `--window-name`, `--keep-window`, `--cwd`, `--tmux-session`, `--tmux-bin`, `--ready-timeout`, `--notify-cmd` (persist a host notifier template on the run; summary via `$LTO_SUMMARY`), `--no-install-hooks`, `--uninstall-hooks`, `--no-runner-constraints` (skip per-runner behavioral-constraints injection: built-in codex block and `$LTO_CONSTRAINTS_DIR`/`~/.config/lto/constraints/<runner>.md` overrides) |
