...
| `dispatch-goal` | Dispatch a goal file to codex, pi, or agy through tmux and print a ready-to-copy completion wait command. | `--runner <runner> --goal <path>` | `--run-id`, `--target`, `--new-window`, `--window-name`, `--keep-window`, `--cwd`, `--tmux-session`, `--tmux-bin`, `--ready-timeout`, `--notify-cmd` (persist a host notifier template on the run; summary via `$LTO_SUMMARY`), `--no-install-hooks`, `--uninstall-hooks` |
