...
| `closeout` | Gate closeout, including base readiness and non-empty-contract target/instrument requirements; by default re-run delivery instruments from the repo root with a 300s timeout per command; update state/run-state, write handoff and changelog. | `--summary` | `--run-id`, `--next-action`, `--blocked-by`, `--allow-dirty`, `--no-changelog`, `--force`, `--reverify-timeout`, `--no-reverify` |
