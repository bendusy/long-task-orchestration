...
| `dispatch-goal` | Dispatch a goal file to codex, pi, or agy through tmux and print a ready-to-copy completion wait command. | `--runner <runner> --goal <path>` | `--run-id`, `--target`, `--new-window`, `--window-name`, `--cwd`, `--tmux-session`, `--tmux-bin`, `--ready-timeout`, `--notify-cmd` (persist a host notifier template on the run; summary via `$LTO_SUMMARY`), `--no-install-hooks`, `--uninstall-hooks` |
| `dispatch-and-wait` | Dispatch a goal AND block until the agent's `agent.turn.completed` fires, then print a summary (dispatch-goal + events --wait in one step). | `--runner <runner> --goal <path>` | all dispatch-goal options + `--timeout <secs>` (default 600) |
| `agent-turn-completed` | Emit an agent turn completion event from a hook, then wake any `events --wait` waiter and optionally signal a human. | None | `--run-id`, `--runner`, `--payload-file`, `--cwd`, `--session-id`, `--summary`, `--rc`, `--source`, `--bell` (ring the terminal/tmux bell), `--notify-cmd` (host notifier template; trusted fields via `{run_id}`/`{runner}`/`{rc}`, untrusted summary via `$LTO_SUMMARY` env to avoid shell injection, e.g. an iaf call) |
