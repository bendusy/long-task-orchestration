...
| `dispatch-goal` | Dispatch a goal file to codex, pi, or agy through the auto-detected multiplexer backend (orca when `$ORCA_TERMINAL_HANDLE`/`$ORCA_WORKSPACE_ID` is set, then herdr when `$HERDR_ENV`/`$HERDR_SOCKET_PATH` is set, tmux otherwise — a GUI multiplexer wins over an inner tmux, whose panes are only visible after attaching; paseo marks no environment of its own and needs an explicit `--backend paseo`; `--backend` overrides) and print a ready-to-copy completion wait command. Windows are retained on success unless `--close-window` is passed, because dispatched output lives only in the terminal scrollback until something collects it. | `--runner <runner> --goal <path>` | `--backend tmux\|herdr\|orca\|paseo`, `--run-id`, `--target`, `--new-window`, `--window-name`, `--close-window` (close an LTO-created window on success; the default retains it), `--keep-window` (no-op, kept for compatibility), `--cwd`, `--tmux-session`, `--tmux-bin`, `--ready-timeout`, `--notify-cmd` (persist a host notifier template on the run; summary via `$LTO_SUMMARY`), `--no-install-hooks`, `--uninstall-hooks`, `--no-runner-constraints` (skip per-runner behavioral-constraints injection: built-in codex block and `$LTO_CONSTRAINTS_DIR`/`~/.config/lto/constraints/<runner>.md` overrides) |
| `dispatch-and-wait` | Dispatch a goal and block for `agent.dispatch.completed` (primary: agent `goal-self-report`; optional side-channels: Codex Stop/`update_goal` proof, pi/agy process-exit with real rc), then print a success/failure summary. Herdr `agent wait`/`agent prompt` idle or done states are transport/readiness only, never task completion. | `--runner <runner> --goal <path>` | all dispatch-goal options + `--timeout <secs>` (default 600) |
