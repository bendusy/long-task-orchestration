...
> 把素材办成公文 / 呈批 / 意见函——多步转换 + 质检 + 呈批的长任务形态。
> LTO 不持有办文领域知识（人名 / 模板 / 公文规范都在 yihub 和 am 办文库）；
> host agent 在 LTO 轨道上**编排 yh cap 链**，LTO 只让它可恢复、可审计、
> 产物可追溯。yh cap 直接当 runner 命令跑，零适配。

触发信号：

- 任务是把会议素材 / 草稿办成正式公文、呈批件、征求意见函，要经多步格式
  转换 + 公文质检 + 呈批链路。
- 产物对外（领导签批 / 跨部门发文），错一个格式/人名/称谓会出事。

可用 primitive：

- 能力发现：`yh cap list`（yihub 0.0.0+，列已注册 cap）、`yh cap <name>
  --describe`（单 cap 元信息）。办文链常用 cap：`convert-to-md`（Word/PDF/图
  转 md）→ `doc-format`（md 解析成结构化 JSON）→ `make-official-doc` /
  `make-approval-doc` / `make-opinion-letter`（生成公文）→ 质检。
- yh cap 当 runner 命令跑：`lto runner --task-id T1 --kind convert
  --command "yh cap convert-to-md --input a.docx --output a.md"`——shell 命令，
  runner 落 exit code + evidence，产物 sha256 进 manifest。
- 质检进 audit 收敛：质检 cap（`gov-doc-verify` / `govdocx-qc` /
  `chengpi-gate`）的 `--json` 输出统一含 `findings[]`（severity 中文
  严重/警告/提示 + claim + location + source_cap + rule）。把它写进 reply
  目录，`lto audit --collect <dir>` 直接吃——LTO 自动把中文 severity 映射到
  四档（严重→critical / 警告→high / 提示→low），按 `--high` / `--critical`
  闸门决定收敛标准。
- 呈批链路：`chengpi-orchestrate`（= `yh chengpi`）/ `official-orchestrate`
  （= `yh official`）是 yh 侧的编排快捷入口；host agent 可整段当一个 runner
  步骤跑，也可拆开逐 cap 跑以便逐步落证据。
- 终稿确认：办文是对外产出，定版走 human gate（见 release 节）。

期望 artifact：

- 每步 yh cap 的输入输出 + exit code（runner evidence）
- 质检 findings ledger（audit collect 后的 audit-ledger.md）
- 最终公文产物的 sha256 指纹（manifest 里可追溯改了哪版）
- 终稿人工确认记录

停止条件：

- 质检收敛：无 critical（严重），high（警告）降到任务约定阈值内，或人类
  override 并记录理由。yh 阈值偏宽（`chengpi-gate` 可能 22 violations 仍
  pass），收敛标准由 LTO 侧 `--high` / `--critical` 闸门定，不照搬 yh 的 pass。
- 人工确认终稿（人名 / 称谓 / 落款 / 格式）后才 closeout。

反模式：

- 跳质检直接出稿。
- 把 yh cap 失败的 exit 1 当成功（runner 会拦，但别手动忽略）。
- 办文产物不留 sha256，出问题无法追溯改了哪版。
- 照搬 yh 质检的 pass/fail 当 LTO 收敛标准（yh 阈值宽，severity 已字段化就
  让 LTO 自己定）。
- 把人名 / 模板 / 公文规范塞进 LTO（那是 yihub 和 am 办文库的领域知识）。
