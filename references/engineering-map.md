# LTO 工程化地图：循环 → 步骤 → 脚本化判断

> 用途：固定 LTO 的长任务循环每一步「能不能用代码兜底」。能脚本化的指向脚本，不能的指向 SKILL.md 规则。
> 这是 SKILL.md 调度逻辑的依据表——宿主推进任务时按这张表决定「这一步该跑哪个脚本 / 该自己判断什么」。
...
| S3a | audit·派工 | 方案 md + auditors | 各 runner 的回复 md | ✅ 全 | bundled `scripts/delegate/` runners |
| S3b | audit·健康 | runner 名单 | 每 runner exit/elapsed/bytes/verdict | ✅ 全 | `lto_run.py preflight` + `scripts/delegate/runners/healthcheck.sh` |
...
| S7 | 落盘 | 本轮决策/天花板/反例 | artifact-memory projection / `docs/decisions/` ADR + manifest | 🟡 半 | `write_decision.py` + `references/decision-logging.md` |
...
| `lto_run.py next` | pattern 路由器（零 LLM） | `[--exec] [--json]` | 决策简报 / argv 命令 | `python3 scripts/lto_run.py next` |
| `lto_run.py autopilot` | 自驱编排（受约束） | `--supervised [--auto-exec] [--decide [--decide-kind] [--decide-budget]]`；`--autonomous` 未实现 | 决策简报 / 沙箱执行 + evidence / 三方收敛 brief | `python3 scripts/lto_run.py autopilot --supervised --decide` |
...
| `scripts/install.sh` | 生成/检查全局 `lto` wrapper | `[--check]` + `LTO_BIN_DIR` | sentinel-managed wrapper；冲突 rc 2 | `bash scripts/install.sh --check` |
...
- **S2/S4 创作**：写方案、写代码本身是 LLM 主体工作，不是 LTO 的职责（LTO 是导航仪不是码农）。
...
| 缺口 | 优先级 | 方案 | 现状落点 |
|---|---|---|---|
| 直接 artifact-memory 写入桥 | 低 | 在 ADR helper 之后另加显式 publish opt-in | 当前 `write_decision.py` 只写 ADR/state/manifest，不碰凭据 |
