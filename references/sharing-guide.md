...
- wrapper 内部同时记录当前 checkout 的 `target/release/lto-rs` 和 `scripts/lto_run.py`。默认走 Rust v2；传 `--use-python` 或 `LTO_USE_PYTHON=1` 才显式回退 legacy Python。仓库移动后重跑安装。
