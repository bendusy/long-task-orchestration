...
> LTO harness 分享说明。LTO 深度集成两个可选 skill（agent-delegate / memory-flow）和项目脚本（deploy.sh）。朋友没有这些也能用核心纪律——下面分清「0 安装能用什么」和「跑满整套要装什么」。
...
- **落盘**：先用 `write_decision.py` 写 repo-local ADR 并登记 artifact；memory-flow MCP/REST 是后续显式集成，不默认碰凭据。

**一句话**：谁当宿主，就把「另外几家」当审计方——这正是异构对抗性的来源，跟宿主是不是 Claude 无关。

#### pi 当宿主 quickstart：host 判断 → LTO/ad 派 codex/agy/claude 异构审计

**完整流程**：pi (DeepSeek) 加载 LTO → pi 按 playbook 判断需要异构审计 → 调用 agent-delegate 派 codex (OpenAI) + agy (Gemini) + claude (Anthropic) → pi 综合三方结论。
