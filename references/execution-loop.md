...
- **`src/commands/ops.rs` autopilot path** — 受约束推进 harness，三档：`--supervised`（出 brief 回吐宿主）/ `--auto-exec`（沙箱或 tmux worker 跑子步骤，retry/stall 刹车）/ `--autonomous`（机械证据闸门 + 机械执行，已实现）。tmux worker carrier 通过 scheduler/tmux runner 每个 pending task 派一个 bounded worker，并用 `.lto/<run>/live/*.worker.json` contract 的 `rc` 推进 `state.tasks`，不是信 worker 自报完成。**autonomous 不 spawn 决策 agent、不替宿主反思**——`operational_reliability` 读跨 run 有界近期事实，`current_run_observability` 核对当前 goal/done_when、instrument 与关联的可解析 runner evidence；先观测后控制，当前 run 无已证实观测信号不放行，`signal_declared`/`missing` 均 `NEEDS_CONFIRM` 早退且不执行。两项都过闸后才机械推进 safe 子步骤；escalate/dangerous/push（含 `git -C . push` 变体）/网络停人类。
