# LTO 执行循环器：runner + judge + parallel/pipeline

> Phase 2 核心机制。runner 执行 task + 自动记录证据，judge 只读审查 + YAML verdict，parallel/pipeline 批量执行多个 task 的校验命令。
...
> 这是**命令批处理**，不是 agent fan-out。需要真正的多 agent 分工请走 bundled delegate（`scripts/delegate/`，codex/pi/agy/claude）。
...
python3 scripts/lto_run.py parallel --phase implementation --concurrency 4

# 并发执行指定 task
python3 scripts/lto_run.py parallel --task-ids T1 T2 T3 --concurrency 3

# 自定义命令
python3 scripts/lto_run.py parallel --kind test --command "pytest tests/ -x" --timeout 600
```
...
python3 scripts/lto_run.py pipeline --phase implementation \
...
> LTO 有两层编排：**shell 层**（runner/parallel/pipeline，跑 pytest/lint）+ **agent 层**（next/autopilot/audit，编排隔离 agent，见文末「Agent 执行层」）。
...
python3 scripts/lto_run.py runner --task-id T1 --command "pytest"

# 显式 opt-in：用仓库真实 git identity 提交 .lto 改动
python3 scripts/lto_run.py runner --task-id T1 --command "pytest" --auto-commit
```
...
- **`next.py`** — pattern 路由器（零 LLM）：`analyze` 状态 → `route` 无歧义给 argv 命令 / escalate 给宿主 LLM 富决策简报。`--exec` 走 shell=False（无注入面）。
...
- **`autopilot.py`** — 自驱动，三档：`--supervised`（出 brief 回吐宿主）/ `--auto-exec`（沙箱跑 safe 子步骤，retry/stall 刹车）/ `--decide`（escalate 时 opt-in spawn 三方异构 agent 跑双轨收敛，出 brief 给宿主读，决策权仍归宿主；配 `--decide-kind direction|review|both` + `--decide-budget`，已实现）。`--autonomous` 下一期未实现。
