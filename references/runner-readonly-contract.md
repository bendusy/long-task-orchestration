...
> 状态：design spec v3（据 R3 复审三家命中的 3 个新 CRITICAL 收口；R2: 3C+8H → R3: 3C+5H，方向已稳，v3 闭合机制具体化后据此改码）
...
| **agy** | `agy --sandbox` | **sandbox 开关**：开 / 不开，无分级 | `agy --help` 有 `--sandbox`；⚠️ 只一次实测「能读、写被限」，**未证明所有写通道（/tmp、~、shell 重定向）全封**（pi+codex MEDIUM）—— 见 §3 边界定义 |
| **claude** | `claude -p --allowedTools <只读集> --permission-mode plan` | **工具白名单** | `claude --help` 有 `--allowedTools`/`--permission-mode`；工具名 PascalCase |
| **pi** | `pi -p --tools read,grep,find,ls` | **工具白名单** | pi `--help` 标「Read-only mode」；⚠️ **未验明 `--tools` 是「完整替换」还是「追加默认集」**（pi HIGH）—— 落地前必须实测，若是追加则不防 bash/edit/write |
...
- **agy**（sandbox-flag）：`--sandbox` 在场 → actual=read-only；缺席 → actual=danger-full-access
  （agy 无中间档，不开 sandbox 即全权限）。于是 approved=read-only 而没开 sandbox → 越权；
  approved=danger-full-access 而没开 → 合规（§3.3 处理 agy+各 approved 组合）。
...
### 3.3 agy 中间级 + 写边界（H4 / MEDIUM）

- agy 只有「sandbox 开/关」二档，**表达不了 `workspace-write`**。若 profile 批准
  `workspace-write` 而 runner 是 agy：agy 不设 `--sandbox` 会走全权限 = 越权（漏判）。
  规则：**agy runner 不支持 `workspace-write` 中间级**——profile approved 为
  workspace-write 且 runner=agy 时，validate 阶段即拒绝（不可兑现的权限组合不放行）。
- agy + approved=`danger-full-access`：合法组合（全权限任务不需 sandbox），不开
  `--sandbox`，actual=danger-full-access ⊆ approved → 合规，不拒绝。
- agy + approved=`read-only`：必须开 `--sandbox`，否则越权（§3.2）。
- agy `--sandbox` 的「写被限」边界需补实测：编辑工具 / shell 重定向 / `/tmp` 写 /
  工作区写是否全部失败。落地前置实测项（见 §5 item 0）。
...
| # | 文件 | 改动 |
|---|---|---|
| 0 | （前置实测）| 实测 ① pi `--tools` 替换 vs 追加语义 ② agy `--sandbox` 写边界 ③ claude `--allowedTools`+`--permission-mode plan` 非交互不挂起且不偷开 Task/Web/子代理。结论写回 §2/§3.3。**任一不过则对应 runner 标 deferred** |
| 1 | `scripts/delegate/runners/agy.sh` | read-only 时 `--dangerously-skip-permissions` → `--sandbox`；写 `<reply>.perm.json`（job_id 绑定，原子 rename） |
...
| 7 | `scripts/lto/plugins.py` validate | agy + approved=workspace-write 组合拒绝（§3.3） |
