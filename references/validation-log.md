...
| BUG-6 pi dispatch-goal TUI posture | `cargo test --locked dispatch_goal -- --nocapture` | runner-plan tests pin pi dispatch-goal to launch `pi --no-skills --no-context-files --no-extensions`, send the normal goal prompt through tmux instead of `--print`, require a probe, avoid reusing ready text such as `ctx` as a post-submit confirmation, and record `completion_event=null` with `completion_mode=manual-pi-tui` until a real pi hook/sentinel completion path exists |
