...
| dispatch-goal completion truth | `cargo test --locked --lib -- --nocapture` | runner-plan tests pin pi/agy to real TUI process-exit wrappers with real rc; Codex Stop remains per-turn unless transcript proof contains `update_goal complete`; only `agent.dispatch.completed` may drive wait/cleanup |
