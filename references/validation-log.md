...
| C5 审计方真审（agy） | agy headless 审 16KB 规格文档 | 真评审非空转 | ✓ 4954 字节，抓出「存量数据走降级分支会空转」「拆事务破坏一致性」「读-改-写之间存在竞态窗口」，全带 (confidence: high)，守反迎合硬约束 |
| C6 审计方真审（pi） | pi/deepseek headless 审 16KB 规格文档（timeout 给足 600s） | 真评审非空转 | ✓ 3886 字节，最强反驳「方案A 只修有伴生对象的行，存量无对象的旧记录在状态变更分支里全是死代码」，带 confidence，守反迎合 |
...
| 环节 | 期望 | 真实结果 |
|---|---|---|
| 多轮上下文保持 | 第 N 轮记得第 1 轮 | ✓ 第 2 轮答出第 1 轮设定的方案代号与审计方名单 |
...
| 宿主读三方 + 收敛判断 | 不投票、亲核 | ✓✓ **超预期**：codex 发现两家对「无伴生对象的存量行」相反定性 → **不数票**，主动去读 spec 与源码核验，**发现代码层某文件已走 agy 方向，但 spec 层对应条目仍写相反验收（代码与 spec 矛盾）**，并**否决 agy 的滚动部署 blocker**（"本仓是单机 stop→cp→start 不是 rolling"）。`Worked 6m 27s` |

**结论**：codex 当宿主**完整多轮长任务真跑通**，且自发做到 skill §3 的 B2(不投票)+B4(亲核否决)——我没教它，它读 skill 自己做到的。收敛质量**超过我之前几轮人工审计**（它抓到的代码↔spec 漂移我没抓到）。证据存档于本机审计产物目录。
...
| pi 读两份 + 收敛 | ✓✓ **质量惊人**：做了 codex/agy 反驳对比表，**不投票**精准识别 3 个共识 blocker 并交叉引用（`codex#3/agy#1` 存量空转、`codex#6/agy#4` 读-改-写竞态导致的脏权威状态、`codex#4/agy#5` 某计数字段漏计），给出「不通过，三 blocker 修完可进开发」+ **分层裁决**（区分运维面 vs spec 逻辑面）。token 真实计费 `$0.033` |

**结论**：pi 当宿主完整长任务真跑通，收敛质量与 codex 当宿主相当甚至更结构化（blocker 交叉引用表）。证据存档于本机审计产物目录。**关键差异**：pi TUI 派工**不需要**放开沙箱（codex 需要），各家宿主的沙箱行为不同。
...
- ✓✓ 收敛：支持 codex 的「新旧版本召回倒挂」Blocker、深挖「无伴生对象的存量行空转」Major、给出 v3 对齐建议（剔除死代码/对齐查询条件/补全生命周期字段）
- 证据存档于本机审计产物目录
...
|---|---|---|---|---|
| codex(OpenAI) | pi+agy | 发现代码层某文件 ↔ spec 层对应条目矛盾，否决 agy rolling blocker | **需放开** `--dangerously-bypass-approvals-and-sandbox` | 直接 `codex` |
...
**最终结论**：pi 不用任何修复（pi.sh 已回退原样），agy 不用修，两家审计方真审 16KB 规格文档都出真评审（pi 3886 字节 / agy 4954 字节，皆带最强反驳 + confidence + 守反迎合）。唯一真实的环境缺口是 claude headless 未登录（待 `/login`）。

### 待补验证（下一轮，不阻塞当前结论）

- [ ] claude headless `/login` 后补一轮 → 四家齐（codex/pi/agy 已坐实，claude 审计方待登录补测）
- [ ] 修 agent-delegate 的 pi deepseek provider headless 链路 → pi 审计方通路优化

### 诚实的范围声明（已更新：C7/C8/C9 追加后）

三家各当宿主已全部真跑通（C7 codex 宿主 / C8 pi 宿主 / C9 agy 宿主，见上方 3/3 结论表）。validation 等级：3/3 实测坐实。唯一未完成项：claude 审计方因 headless 未登录未实测（待 `/login`），不影响三家宿主结论。
